...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        $stmt = $pdo->prepare("
            INSERT INTO movies (title, description, poster_url, genre, is_series, language)
            VALUES (?, ?, ?, ?, ?, ?)
        ");
        // 'is_series' is false, 'language' is set
        $stmt->execute([$title, $description, $poster_url, $genre, false, $language]);
